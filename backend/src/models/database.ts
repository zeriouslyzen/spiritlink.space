import { Pool } from 'pg';
import { TemporalLayer, DomainVector, AstralEntityType, EmergenceEventType } from '../types/consciousness';
...
export let PGVECTOR_AVAILABLE = false;
...
export async function initializeDatabase() {
  ...
  try {
    ...
    try { await client.query(`CREATE EXTENSION IF NOT EXISTS btree_gin;`); } catch {}

    try {
      const chk = await client.query(`SELECT 1 FROM pg_extension WHERE extname='vector'`);
      PGVECTOR_AVAILABLE = ((chk?.rowCount ?? 0) > 0);
    } catch {
    ...
    await client.query(`
      ...
      ${PGVECTOR_AVAILABLE ? "CREATE INDEX IF NOT EXISTS idx_chunks_sparse_gin ON chunks USING GIN (sparse_tsv);" : ''}
      ${PGVECTOR_AVAILABLE ? "CREATE INDEX IF NOT EXISTS idx_chunks_lang ON chunks(lang);" : ''}
      ${PGVECTOR_AVAILABLE ? "CREATE INDEX IF NOT EXISTS idx_documents_owner ON documents(owner_id);" : ''}
    `);
...
export class ConsciousnessNodeModel {
  ...
  static async findByConcept(concept: string) {
    const query = 'SELECT * FROM consciousness_nodes WHERE concept ILIKE $1';
    const result = await pool.query(query, [`%${concept}%`]);
    return result.rows;
  }

  static async findByTemporalLayer(temporalLayer: TemporalLayer) {
    const query = 'SELECT * FROM consciousness_nodes WHERE temporal_layer = $1';
    const result = await pool.query(query, [temporalLayer]);
    return result.rows;
  }

  static async findByDomainVector(domainVector: DomainVector) {
    const query = 'SELECT * FROM consciousness_nodes WHERE domain_vector = $1';
    const result = await pool.query(query, [domainVector]);
    return result.rows;
  }
}

// Emergence Event operations
export class EmergenceEventModel {
  static async create(data: {
  ...
  static async findByEventType(eventType: EmergenceEventType) {
...
export class LiveConsciousnessModel {
  static async create(data: {
    userId: string;
